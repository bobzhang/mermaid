...
function parseCliOptions(args: string[]): SweepCliOptions {
  ...
  return { trialCounts, sweepPassCounts, sweepKernels, jsonPath }
}

function candidateKey(config: CandidateConfig): string {
  return `${config.sweepKernel}|${config.trialCount}|${config.sweepPassCount}`
}
...
function main(): void {
  ...
  for (const sweepKernel of options.sweepKernels) {
    for (const trialCount of options.trialCounts) {
      for (const sweepPassCount of options.sweepPassCounts) {
        enqueue({ trialCount, sweepPassCount, sweepKernel })
      }
    }
  }
  ...
  for (const config of configs) {
    console.log(
      `run kernel=${config.sweepKernel} trial=${config.trialCount} sweep=${config.sweepPassCount}`,
    )
  ...
  console.log(
    `baseline kernel=${baseline.config.sweepKernel} trial=${baseline.config.trialCount} sweep=${baseline.config.sweepPassCount} rank=${baseline.rankOrderMismatch}/${baseline.rankOrderComparable} post=${baseline.postSweepOrderMismatch}/${baseline.postSweepOrderComparable} final=${baseline.finalOrderMismatch}/${baseline.finalOrderComparable} wg=${baseline.weightedGap.toFixed(4)} lc=${baseline.logicalCrossingMultiplier.toFixed(4)} structural_ok=${baseline.structuralOk}/${baseline.fixtures}`,
  )
